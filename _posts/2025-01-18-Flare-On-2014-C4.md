...
The `shellcode.exe` executable was loaded into **x32dbg**. Here, comments were added to improve understanding of its behavior. The shellcode uses `LoadLibraryA` to load `user32.dll` at offset `0x004132F`. Then it calls `GetProcAddress` at offset `0x00401348` to obtain the address of `MessageBoxA` and execute it at offset `0x004013DD`.

![Debugging](/images/2025-01-18-Flare-On-2014-C4/2.png)
...
![Debugging](/images/2025-01-18-Flare-On-2014-C4/3.png)

The memory dump of `edx` can be seen below. 

![Debugging](/images/2025-01-18-Flare-On-2014-C4/4.png)
...
For example, consider the instruction `xor dword ptr ds:[edx+B], 32FBA316`. The memory at `[edx+B]` initially contains the value `79CEBE32` as can be seen in dump. 
Below this instruction is `push 32BECE79`. Note its opcode is `68 79CEBE32`. 
...
![Debugging](/images/2025-01-18-Flare-On-2014-C4/5.png)
