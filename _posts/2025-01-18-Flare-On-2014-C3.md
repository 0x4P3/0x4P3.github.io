...
The third challenge `such_evil` was loaded in IDA for analysis and navigated to:
`start` > `sub_401000`

Within `sub_401000`, an indirect function call was found. Values were sequentially moved into memory locations `var_1` through `var_201`. Ultimately, `var_201` contained the code, and its memory address was loaded into the `eax` register, which was then invoked by a `call` instruction to execute the code as can be seen below.
