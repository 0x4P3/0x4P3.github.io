...
- At line 2 and 5, `____SSS[____SS].subject` and `____S5 = ____SSS[____$S].subject` reference the subject value of two annotation objects; indices 0 and 1 respectively.
- At line 4, all occurance of `X_17844743X_170987743` in index 0 annotation's subject value is replaced with `%` character. 
- At line 6, all occurance of `89af50d` in index 1 annotation's subject value is replaced with `%` character, and the resulting data is appended to earlier result.
