...
The extension listen for `submit` event on `www.facebook.com`, reads the submitted form via `FormData()`, and extracts `username`/`email` and `password` fields. Then it exfiltrate the collected credentials via `exfiltrateCredentials()`.
